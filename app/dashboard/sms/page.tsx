...
import {
    Users, MessageSquare, TrendingUp, BarChart3,
    Send, Info, Mail, MessageCircle, Phone, LayoutDashboard,
...
export default function SmsDashboardPage() {
    ...
    const fetchData = useCallback(async (from: Date, to: Date) => {
        setLoading(true);
        try {
            const res = await fetch(`/api/sms-leads?from=${encodeURIComponent(startOfDay(from).toISOString())}&to=${encodeURIComponent(endOfDay(to).toISOString())}`);
    ...
    const stats = useMemo(() => {
        if (!smsData) return { totalLeads: 0, sentCount: 0, uniqueSentCount: 0, totalReplies: 0, ownerReachouts: 0, ownerReplies: 0, ownerSent: 0, dailyTrend: [] as any[] };
        const allLeads = [...(smsData.nr_wf || []), ...(smsData.followup || []), ...(smsData.nurture || [])];
    ...
    const donutData = [
        ...
        { name: 'Replies',        value: stats.totalReplies,    color: '#D6336C'  },
    ];
    ...
    return (
        <div style={{ display: 'flex', flexDirection: 'column', gap: 24, paddingBottom: 40, position: 'relative', minHeight: 500 }}>
            ...
            {/* Key Metrics */}
            {(() => {
                const crmUnique = Math.max(0, stats.uniqueSentCount - stats.ownerReachouts);
                const crmReplies = Math.max(0, stats.totalReplies - stats.ownerReplies);
                const crmSent = Math.max(0, stats.sentCount - stats.ownerSent);
                const crmRate = crmUnique > 0 ? ((crmReplies / crmUnique) * 100).toFixed(1) : '0';

                const genUnique = stats.ownerReachouts;
                const genReplies = stats.ownerReplies;
                const genSent = stats.ownerSent;
                const genRate = genUnique > 0 ? ((genReplies / genUnique) * 100).toFixed(1) : '0';
                ...
                return (
                    <div>
                        ...
                        <BusinessSection
                            ...
                            loading={loading}
                            metrics={[
                                { title: "Unique Msg Sent", value: crmUnique.toLocaleString(), accentColor: "var(--purple)", icon: <Users size={17} /> },
                                { title: "Total Replies", value: crmReplies.toLocaleString(), accentColor: "#D6336C", icon: <MessageSquare size={17} />, info: `${crmRate}% Response Rate` },
                                { title: "Messages Sent", value: crmSent.toLocaleString(), accentColor: "var(--blue)", icon: <Send size={17} /> },
                            ]}
                        />
                        ...
                        <BusinessSection
                            ...
                            loading={loading}
                            metrics={[
                                { title: "Unique Msg Sent", value: genUnique.toLocaleString(), accentColor: "var(--purple)", icon: <Users size={17} /> },
                                { title: "Total Replies", value: genReplies.toLocaleString(), accentColor: "#D6336C", icon: <MessageSquare size={17} />, info: `${genRate}% Response Rate` },
                                { title: "Messages Sent", value: genSent.toLocaleString(), accentColor: "var(--blue)", icon: <Send size={17} /> },
                            ]}
                        />
                    </div>
                );
                    ...
                    <div style={{ height: 200 }}>
                        <ResponsiveContainer width="100%" height="100%">
                            <LineChart data={trendData} margin={{ top: 10, right: 10, left: 0, bottom: 0 }}>
                                ...
                                <Line type="monotone" dataKey="replied" name="Replied" stroke="#D6336C" strokeWidth={2} dot={{ r: 3, fill: '#D6336C', strokeWidth: 0 }} activeDot={{ r: 5 }} />
                            </LineChart>
                        </ResponsiveContainer>
                    </div>
                    {/* Legend */}
                    <div style={{ display: 'flex', justifyContent: 'center', gap: 24, marginTop: 12 }}>
                        {[{ color: 'var(--blue)', label: 'Sent' }, { color: '#D6336C', label: 'Replied' }].map(item => (
