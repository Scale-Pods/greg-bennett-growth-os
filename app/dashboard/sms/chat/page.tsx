...
const getMsgDateWithFallback = (lead: any, msgKey: string, tsKey?: string) => {
    ...
    // 3. Fall back to the lead's "Created At"
...
// Extract ISO timestamp embedded in message text, fall back to Whatsapp_1_Date
const extractOwnerSmsDate = (o: any): Date | null => {
...
const getOwnerLatestActivity = (o: any) => {
    const waDate = extractOwnerSmsDate(o);
...
async function fetchSmsLeadsData(from: Date, to: Date): Promise<{ nr_wf: any[]; followup: any[]; nurture: any[]; owners: any[] }> {
...
export default function SmsChatPage() {
    ...
    const [smsOwners, setSmsOwners] = useState<any[]>([]);
    const [loadingSms, setLoadingSms] = useState(true);
    const loading = loadingSms;
    ...
    useEffect(() => {
        ...
        setLoadingSms(true);
        fetchSmsLeadsData(dateRange.from, dateRange.to || dateRange.from)
            .then(data => {
                ...
                setSmsOwners(data.owners || []);
            })
            .catch(err => console.error("[SMS chat]", err))
    ...
    const [activeTab, setActiveTab] = useState<"bootcamps" | "realty" | "wealth" | "coaching">("bootcamps");
    ...
    useEffect(() => {
        ...
        if (initialTab === 'realty') setActiveTab('realty');
        else if (initialTab === 'wealth') setActiveTab('wealth');
        else if (initialTab === 'coaching') setActiveTab('coaching');
        
        if (initialSelectedId) {
            if (initialTab === 'realty' || initialTab === 'wealth' || initialTab === 'coaching') {
                if (smsOwners.length > 0) {
                    const found = smsOwners.find(o =>
    ...
    }, [initialSelectedId, initialTab, smsOwners]);
        ...
        return leadsInRange.filter(l => {
            ...
            const matchesTemplate = activeFilters.templates.length === 0 ||
                activeFilters.templates.some(tName => {
                    const match = tName.match(/SMS\s*#?\s*(\d+)/i);
        ...
        return smsOwners.filter(o => {
            ...
            if (activeFilters.templates.length > 0) {
                const matchesTemplate = activeFilters.templates.some(tName => {
                    ...
                    const isIntro = tName.toLowerCase().includes("cold") || tName.toLowerCase().includes("intro") || tName.toLowerCase().includes("owner") || tName.toLowerCase().includes("lead");
                    const isFollowUp = tName.toLowerCase().includes("follow-up") || tName.toLowerCase().includes("followup");
                    const isNurture = tName.toLowerCase().includes("nurture");
    ...
    }, [smsOwners, searchQuery, activeFilters]);
    ...
    const stats = useMemo(() => {
        ...
        return {
            totalLeads: activeTab === "bootcamps" ? filteredLeads.length : filteredOwners.length,
            sentCount,
            uniqueSentCount,
            repliedCount,
            failedCount,
    ...
    const totalPages = activeTab === "bootcamps" 
    ...
    return (
        <div className="space-y-5 pb-10 relative min-h-[500px]">
            {(activeTab === "bootcamps" ? loading : loadingSms) && <BennettLoader />}
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                <div>
                    <h1 style={{ fontSize: 22, fontWeight: 700, letterSpacing: 'var(--ls-heading)', color: 'var(--label-primary)' }}>SMS Chats</h1>
                        ...
                        {[
                            ...
                            <button
                                ...
                                style={{
                                    ...
                                    background: activeTab === tab.key ? 'var(--bg-layer1)' : 'transparent',
                                    color: activeTab === tab.key ? tab.color : 'var(--label-secondary)',
            ...
            <div className="flex lg:hidden items-center gap-2">
                ...
                >
                    ...
                    {(activeFilters.replyStatus.length > 0 || activeFilters.loops.length > 0 || activeFilters.messageStatus.length > 0 || activeFilters.templates.length > 0) && (
                        <span style={{ width: 7, height: 7, borderRadius: '50%', background: '#D6336C', marginLeft: 2 }} />
                    )}
                </button>
                {(activeFilters.replyStatus.length > 0 || activeFilters.loops.length > 0 || activeFilters.messageStatus.length > 0 || activeFilters.templates.length > 0) && (
                    <button onClick={handleResetFilters} style={{ fontSize: 11, fontWeight: 700, color: '#D6336C', background: 'none', border: 'none', cursor: 'default' }}>RESET ALL</button>
                )}
            </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-4 gap-5">
                    ...
                    <div className="liquid-card" style={{ padding: '16px', display: 'flex', flexDirection: 'column', gap: 20 }}>
                        ...
                        {activeTab !== "bootcamps" && (
                            <FilterSection title="Templates">
                                <FilterOption 
                                    ...
                                    label={
                                        <div className="flex flex-col">
                                            <span className="text-[11px] font-bold">Generated Lead SMS #1</span>
                        ...
                        <button
                            ...
                            style={{
                                ...
                                background: '#D6336C', color: '#fff', fontSize: 13, fontWeight: 600,
                ...
                <div className="lg:col-span-3 space-y-4">
                    <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-3">
                        ...
                        <MetricCard title="SMS Sent" value={(activeTab === "bootcamps" ? loading : loadingSms) ? "..." : stats.sentCount.toLocaleString()} desc="Total outgoing SMS pulses" icon={Send} />
                            <MetricCard title="Unique Msg Sent" value={(activeTab === "bootcamps" ? loading : loadingSms) ? "..." : stats.uniqueSentCount.toLocaleString()} desc="Unique entities contacted" icon={Users} />
                            <MetricCard title="Total Replies" value={(activeTab === "bootcamps" ? loading : loadingSms) ? "..." : stats.repliedCount.toLocaleString()} desc={`${stats.responseRate}% Response Rate`} icon={MessageSquare} />
                        </div>
                        <div className="liquid-card" style={{ padding: '14px 16px' }}>
                            ...
                            <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
                                ...
                                <StatusBar label="Replied" value={stats.repliedCount} total={stats.uniqueSentCount || 1} color="#D6336C" />
                                {stats.failedCount > 0 && (
                    ...
                    <div style={{ position: 'relative' }}>
                        ...
                        <Input className="pl-10" style={{ background: 'var(--fill-tertiary)', border: '1px solid var(--glass-border)', color: 'var(--label-primary)', borderRadius: 'var(--radius-lg)' }} placeholder={`Search ${activeTab === "bootcamps" ? "Bennett Bootcamps" : activeTab === "realty" ? "Realty Solutions" : activeTab === "wealth" ? "Wealth Builder" : "Elite Coaching"} by name or phone...`} value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} />
                    </div>
                        ...
                        {activeTab === "bootcamps" ? (
                            <>
                                {loading ? (
                                    <div style={{ padding: '40px 16px', textAlign: 'center', color: 'var(--label-tertiary)', display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 8 }}>
                                        <RefreshCw style={{ width: 18, height: 18, color: '#D6336C' }} className="animate-spin" />
                                        <span style={{ fontSize: 13 }}>Loading real-time chats...</span>
                                    </div>
                                ) : filteredLeads.length === 0 ? (
                                    <div style={{ padding: '40px 16px', textAlign: 'center', color: 'var(--label-tertiary)', fontSize: 13 }}>No SMS chats found.</div>
                                ) : (
                                    <TooltipProvider>
                            ...
                            <>
                                {loadingSms ? (
                                    ...
                                    <div style={{ padding: '40px 16px', textAlign: 'center', color: 'var(--label-tertiary)', fontSize: 13 }}>No generated leads SMS chats found.</div>
                                ) : (
                                    <TooltipProvider>
                                        <div className="overflow-x-auto">
                                            <table className="w-full text-left text-sm min-w-[650px]">
                                                <thead style={{ borderBottom: '1px solid var(--hairline)' }}>
                                                    <tr style={{ background: 'var(--fill-quaternary)' }}>
                                                        ...
                                                        <th style={{ padding: '10px 16px', fontSize: 11, fontWeight: 600, textTransform: 'uppercase', letterSpacing: '0.06em', color: 'var(--label-tertiary)', textAlign: 'center' }}>Reply Track</th>
                                                        <th style={{ padding: '10px 16px', fontSize: 11, fontWeight: 600, textTransform: 'uppercase', letterSpacing: '0.06em', color: 'var(--label-tertiary)', textAlign: 'center' }}>Status</th>
                                                ...
                                                <tbody className="divide-y divide-slate-100">
                                                    {paginatedOwners.map((owner, idx) => {
                                                        ...
                                                        const wpDate = extractOwnerSmsDate(owner);
                                                        return (
                        ...
                        {totalPages > 1 && (
                            <div style={{ borderTop: '1px solid var(--hairline)', padding: '10px 16px', display: 'flex', alignItems: 'center', justifyContent: 'space-between', background: 'var(--fill-quaternary)' }}>
                                <div style={{ fontSize: 12, color: 'var(--label-tertiary)', fontWeight: 500 }}>
                                    Showing <span style={{ color: 'var(--label-primary)', fontWeight: 700 }}>{(currentPage - 1) * leadsPerPage + 1}</span> – <span style={{ color: 'var(--label-primary)', fontWeight: 700 }}>{Math.min(currentPage * leadsPerPage, (activeTab === "bootcamps" ? filteredLeads.length : filteredOwners.length))}</span> of <span style={{ color: 'var(--label-primary)', fontWeight: 700 }}>{(activeTab === "bootcamps" ? filteredLeads.length : filteredOwners.length)}</span> {activeTab === "bootcamps" ? "Bootcamps Leads" : activeTab === "realty" ? "Realty Leads" : activeTab === "wealth" ? "Wealth Builder Leads" : "Coaching Leads"}
                                </div>
    ...
    return (
        <div className="liquid-card" style={{ padding: '14px 16px' }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 8 }}>
                <div style={{ padding: 7, borderRadius: 'var(--radius-sm)', background: 'rgba(214,51,108,0.10)', color: '#D6336C', flexShrink: 0 }}>
    ...
    return (
        <button
            ...
            style={{
                ...
                background: checked ? 'rgba(214,51,108,0.10)' : 'transparent',
                border: `1px solid ${checked ? 'rgba(214,51,108,0.25)' : 'transparent'}`,
        ...
        >
            <span style={{
                ...
                background: checked ? '#D6336C' : 'var(--fill-secondary)',
                border: `1.5px solid ${checked ? '#D6336C' : 'var(--hairline)'}`,
                transition: 'all 120ms',
            }} />
            <span style={{ fontSize: 12, fontWeight: checked ? 600 : 400, color: checked ? '#D6336C' : 'var(--label-secondary)', flex: 1 }}>{label}</span>
        </button>
    );
}
...
function CustomerRow({ lead: leadRaw, onClick }: { lead: any; onClick: () => void }) {
    ...
    return (
        <tr style={{ borderBottom: '1px solid var(--hairline)', cursor: 'pointer', transition: 'background 100ms' }}
            ...
            <td style={{ padding: '10px 16px', textAlign: 'center' }}>
                <span style={{ display: 'inline-block', padding: '2px 8px', borderRadius: 'var(--radius-full)', fontSize: 10, fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.04em', background: 'rgba(214,51,108,0.10)', color: '#D6336C', border: '1px solid rgba(214,51,108,0.20)' }}>{lead.source_loop}</span>
            </td>
                ...
                <TooltipProvider>
                    <Tooltip>
                        <TooltipTrigger asChild>
                            <div>
                                {hasReplied ? (
                                    <Badge className="bg-pink-100 text-pink-700 hover:bg-pink-100 border-none text-[10px] font-bold">REPLIED</Badge>
                                ) : (
...
function MessageStatusBadge({ index, status }: { index: number, status: string }) {
    ...
    if (formatted.includes("Read")) badgeClass = "bg-pink-50 text-pink-700 border-pink-100"; // Changed read status to pink for SMS theme
