...
function getOwnerDate(owner: SmsOwner): Date | null {
...
function getLeadDate(lead: SmsLead): Date | null {
...
export default function SmsLeadsPage() {
    ...
    const [smsOwners, setSmsOwners] = useState<SmsOwner[]>([]);
    ...
    const [selectedOwnerObj, setSelectedOwnerObj] = useState<SmsOwner | null>(null);
    ...
    // Fetch from the dedicated SMS leads endpoint
    const fetchSmsData = useCallback(async (from: Date, to: Date) => {
        ...
        try {
            ...
            const nr_wf: SmsLead[] = (data.nr_wf || []).map((l: any) => ({ ...l, source_loop: "Intro" }));
            const followup: SmsLead[] = (data.followup || []).map((l: any) => ({ ...l, source_loop: "Follow Up" }));
            const nurture: SmsLead[] = (data.nurture || []).map((l: any) => ({ ...l, source_loop: "Nurture" }));
            setSmsLeads([...nr_wf, ...followup, ...nurture]);
            setSmsOwners(data.owners || []);
        } catch (err) {
            console.error("[SMS leads]", err);
        } finally {
            setLoading(false);
        }
    }, []);
    ...
    return (
        <div className="space-y-5 pb-10 relative min-h-[500px]">
            ...
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                <div>
                    <h1 style={{ fontSize: 22, fontWeight: 700, letterSpacing: 'var(--ls-heading)', color: 'var(--label-primary)' }}>SMS Leads</h1>
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: 8, flexWrap: 'wrap' }}>
                    <div style={{ display: 'flex', background: 'var(--fill-tertiary)', borderRadius: 'var(--radius-md)', padding: 3, gap: 2 }}>
                        ...
                        <button
                            ...
                            style={{ padding: '5px 14px', borderRadius: 'var(--radius-sm)', fontSize: 12, fontWeight: 600, border: 'none', cursor: 'default', display: 'flex', alignItems: 'center', gap: 5, transition: 'all 130ms', background: activeTab === "owners" ? 'var(--bg-layer1)' : 'transparent', color: activeTab === "owners" ? '#D6336C' : 'var(--label-secondary)', boxShadow: activeTab === "owners" ? 'var(--shadow-sm)' : 'none' }}
                ...
                <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap' }}>
                    <DropdownMenu>
                        <DropdownMenuTrigger asChild>
                            <button style={{ display: 'flex', alignItems: 'center', gap: 6, padding: '6px 12px', borderRadius: 'var(--radius-md)', border: '1px solid var(--glass-border)', background: activeFilters.replyStatus.length > 0 ? 'rgba(214,51,108,0.10)' : 'var(--fill-tertiary)', color: activeFilters.replyStatus.length > 0 ? '#D6336C' : 'var(--label-secondary)', fontSize: 13, fontWeight: 500, cursor: 'default' }}>
                    ...
                    {activeTab === "leads" && (
                        <DropdownMenu>
                            <DropdownMenuTrigger asChild>
                                <button style={{ display: 'flex', alignItems: 'center', gap: 6, padding: '6px 12px', borderRadius: 'var(--radius-md)', border: '1px solid var(--glass-border)', background: activeFilters.loops.length > 0 ? 'rgba(214,51,108,0.10)' : 'var(--fill-tertiary)', color: '#D6336C', fontSize: 13, fontWeight: 500, cursor: 'default' }}>
                    ...
                    <button
                        ...
                        onClick={() => { if (dateRange?.from) fetchSmsData(dateRange.from, dateRange.to || dateRange.from); }}
                    >
                    ...
                    <table className="w-full text-left">
                        <thead style={{ borderBottom: '1px solid var(--hairline)' }}>
                            <tr style={{ background: 'var(--fill-quaternary)' }}>
                                ...
                                <th style={{ padding: '10px 16px', fontSize: 11, fontWeight: 600, textTransform: 'uppercase', letterSpacing: '0.06em', color: 'var(--label-tertiary)' }}>SMS Date</th>
                                ...
                                loading ? (
                                    <tr>
                                        <td colSpan={7} style={{ padding: '80px 16px', textAlign: 'center', color: 'var(--label-tertiary)' }}>
                                            <RefreshCw style={{ width: 20, height: 20, margin: '0 auto 8px', animation: 'spin 1s linear infinite', color: '#D6336C' }} />
                                            Loading SMS leads...
                                        </td>
                                    </tr>
                                        ...
                                        return (
                                            ...
                                            >
                                                ...
                                                <td style={{ padding: '12px 16px' }}>
                                                    <span style={{ display: 'inline-flex', alignItems: 'center', padding: '2px 8px', borderRadius: 'var(--radius-sm)', fontSize: 10, fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.05em', background: 'rgba(214,51,108,0.10)', color: '#D6336C' }}>
                                                        {lead.source_loop || "—"}
                                                    </span>
                                                </td>
                                    ...
                                    paginatedOwners.map((owner, index) => {
                                        ...
                                        return (
                                            <tr
                                                ...
                                                onMouseEnter={e => (e.currentTarget.style.background = 'var(--fill-quaternary)')}
                                                onMouseLeave={e => (e.currentTarget.style.background = 'transparent')}
                                                onClick={() => { setSelectedOwnerObj(owner); }}
                                            >
                                                <td style={{ padding: '12px 16px' }} onClick={(e) => e.stopPropagation()}>
                                                    <Checkbox />
                                                </td>
            ...
            {/* Generated Lead Chat Detail Modal */}
            <Dialog open={!!selectedOwnerObj} onOpenChange={(open) => { if (!open) setSelectedOwnerObj(null); }}>
                <DialogContent className="max-w-4xl max-h-[90vh] overflow-hidden p-6 gap-0 apple-dialog">
                    <DialogHeader className="sr-only">
                        <DialogTitle>Generated Lead SMS Chat Detail</DialogTitle>
                    </DialogHeader>
                    {selectedOwnerObj && (
                        <SmsOwnerChatDetail
                            owner={selectedOwnerObj}
                            onClose={() => setSelectedOwnerObj(null)}
                        />
                    )}
                </DialogContent>
            </Dialog>
